...
RegisterWithFacebook::RegisterWithFacebook(QObject *parent) : QObject(parent)
{
  ...
  connect(&oauth2, authorizeWithBrowser(QUrl), this, onAuthorizeWithBrowser(QUrl));
  connect(&oauth2, statusChanged(QAbstractOAuth::Status), this, onStatusChanged(QAbstractOAuth::Status));
  connect(&oauth2, granted(), this, onAuthorizationGranted());
}
